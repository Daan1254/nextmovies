...
const seats: SeatInterface[] = Array.from({ length: 36 }, (_, index) => {
...
export default function Page() {
  const [reservedSeats, setReservedSeats] = useState<SeatInterface[]>([]);

  const handleToggle = (seat: SeatInterface) => {
    if (reservedSeats.some((reservedSeat) => reservedSeat.uuid === seat.uuid)) {
      setReservedSeats(
        reservedSeats.filter((reservedSeat) => reservedSeat.uuid !== seat.uuid)
      );
    } else {
  ...
  const handleOrder = () => {
    const orderSeats: SeatInterface[] = reservedSeats.map(
      (seat: SeatInterface) => {
        return {
          uuid: seat.uuid,
          column: seat.column,
  ...
  return (
    <main className="grid place-items-center">
      <div className="group relative p-8">
        ...
        <div className="absolute bottom-0 left-0 p-10 text-white text-sm lg:text-base">
          ...
          <p className="mt-1 text-sm italic text-gray-200 line-clamp-3">
            ...
            Accusantium illo iste dicta, repellat, odio sapiente eveniet libero
            praesentium, nemo voluptatem similique nam nostrum amet quos debitis
            odit quam ullam consequuntur!
          </p>
        </div>
      </div>
      <div
      ...
      <div className="w-2/3">
        ...
        <button
          className="ml-auto border border-white rounded-lg w-max mb-16 px-4 py-1 flex flex-row gap-4 hover:border-black hover:bg-white hover:text-black transition-all duration-300"
          onClick={handleOrder}
        >
          Reserve Seats
        </button>
